<div class="conteneurZoneCommentaires row">
    <div class="commentaires col-md-6">
        ...
        @foreach($arrCommentaires as $commentaire)
            <div class="commentaire">
                <h3>{{ $commentaire -> titre_commentaire }}</h3>
                <p class="auteur"><b>Commentaire par: </b>{{ $commentaire -> prenom }} {{ $commentaire -> nom }}</p>
                <div class="zoneEtoiles zoneEtoiles{{ $commentaire -> cote }}"></div>
                <p>{{ $commentaire -> texte_commentaire }}</p>
                @if($commentaire -> achat_verifie)
                    <p><span class="achatVerif">Achat vérifié</span></p>
                @endif
            </div>
        @endforeach
